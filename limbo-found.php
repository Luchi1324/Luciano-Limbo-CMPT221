<!--
Report found items page for Limbo
...
<body>
  ...
  <div class = 'header'>
    ...
    <p>If you found something, you're in luck: this is the place to report it!</p>
  </div>
  ...
  <?php
    ...
    if ($_SERVER[ 'REQUEST_METHOD' ] == 'POST') {
      ...
      # Validates input
      if (empty($description) or empty($identifiers) or empty($create_date) or empty($room) or empty($finder)) {
        echo '<p style="color:red">Please fill in any empty fields.</p>' ;
      } else {
        $result = insert_record($dbc, $location, $description, $identifiers, $create_date, $create_date, $room, '', $finder, 'Found', $bounty) ;
  ...
  <form action="limbo-found.php" method="POST">
    <table>
    <tr>
    ...
    <tr>
      <td>Date Found: </td><td><input type="date" name="date" placeholder="YYYY-MM-DD"></td>
    </tr>
    <tr>
      <td>Time Found: </td><td><input type="time" name="time" placeholder="HH:MM"></td>
    </tr>
    <tr>
      <td><label>Location Found: </label></td>
    ...
    <tr>
      <td>Room Found in: </td><td><input type="text" name="room" placeholder="HC 2020, LWC P4"></td>
    </tr>
    <tr>
      <td>Bounty: </td><td><input type="number" name="bounty" value="0"></td>
    </tr>
    <tr>
      <td>Name & Contact: </td><td><textarea type="text" name="finder" placeholder="John Doe, text at 123-456-7789""></textarea></td>
    </tr>
    </table>
